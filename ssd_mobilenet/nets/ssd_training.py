...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        num_boxes = tf.cast(tf.shape(y_true)[1],tf.float64)
        ...
        # 获得一个1.0
        has_min = tf.cast(tf.reduce_any(pos_num_neg_mask),tf.float64)
        ...
        # 取top_k个置信度，作为负样本
        _, indices = tf.nn.top_k(max_confs * (1 - y_true[:, :, -1]),
                                 k=num_neg_batch)
        ...
        # total_loss = tf.Print(total_loss,[tf.reduce_sum(pos_conf_loss)/tf.reduce_sum(num_pos),tf.reduce_sum(neg_conf_loss)/tf.reduce_sum(num_pos),tf.reduce_sum(self.alpha * pos_loc_loss) / tf.reduce_sum(num_pos)])
        return total_loss
    ...
    def generate(self, train=True):
        while True:
            ...
            for annotation_line in lines:  
                ...
                if len(y)==0:
                    continue
                boxes = np.array(y[:,:4],dtype=np.float64)
