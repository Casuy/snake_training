...
def focal(alpha=0.25, gamma=2.0):
    def _focal(y_true, y_pred):
        ...
        # 找出存在目标的先验框
        indices_for_object        = tf.where(tensorflow.keras.backend.equal(anchor_state, 1))
        ...
        # 计算每一个先验框应该有的权重
        alpha_factor_for_object = tensorflow.keras.backend.ones_like(labels_for_object) * alpha
        ...
        # 将权重乘上所求得的交叉熵
        cls_loss_for_object = focal_weight_for_object * tensorflow.keras.backend.binary_crossentropy(labels_for_object, classification_for_object)

        # 找出实际上为背景的先验框
        indices_for_back        = tf.where(tensorflow.keras.backend.equal(anchor_state, 0))
        labels_for_back         = tf.gather_nd(labels, indices_for_back)
        classification_for_back = tf.gather_nd(classification, indices_for_back)

        # 计算每一个先验框应该有的权重
        alpha_factor_for_back = tensorflow.keras.backend.ones_like(labels_for_back) * (1-alpha)
        focal_weight_for_back = classification_for_back
        focal_weight_for_back = alpha_factor_for_back * focal_weight_for_back ** gamma

        # 将权重乘上所求得的交叉熵
        cls_loss_for_back = focal_weight_for_back * tensorflow.keras.backend.binary_crossentropy(labels_for_back, classification_for_back)

        # 标准化，实际上是正样本的数量
        normalizer = tf.where(tensorflow.keras.backend.equal(anchor_state, 1))
        normalizer = tensorflow.keras.backend.cast(tensorflow.keras.backend.shape(normalizer)[0], tensorflow.keras.backend.floatx())
        normalizer = tensorflow.keras.backend.maximum(tensorflow.keras.backend.cast_to_floatx(1.0), normalizer)

        # 将所获得的loss除上正样本的数量
        cls_loss_for_object = tensorflow.keras.backend.sum(cls_loss_for_object)/normalizer
        cls_loss_for_back = tensorflow.keras.backend.sum(cls_loss_for_back)/normalizer
