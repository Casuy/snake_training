        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    # 进行iou的非极大抑制
                    self.boxes = tf.cast(boxes_to_process,dtype=tf.float32)
                    self.scores = tf.cast(confs_to_process,dtype=tf.float32)
                    idx = self.nms = tf.image.non_max_suppression(self.boxes, self.scores,
