...
def shift(shape, stride, anchors):
    ...
    shifted_anchors = np.reshape(anchors, [1, number_of_anchors, 4]) + np.array(np.reshape(shifts, [k, 1, 4]), tf.keras.backend.floatx())
    shifted_anchors = np.reshape(shifted_anchors, [k * number_of_anchors, 4])
    # print(np.shape(shifted_anchors))
    # print(shifted_anchors)
    return shifted_anchors
def get_anchors(model):
    border = model.layers[0].output_shape[0][1]
