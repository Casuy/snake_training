...
sys.setrecursionlimit(40000) # 设置python解释器最大的栈数

parser = OptionParser()
#训练文件存放路径，每行一个文件中的一个物体，格式为：img_path,xmin,ymin,xmax,ymax,class_name
parser.add_option("-p", "--path", dest="train_path", help="Path to training data.",default='./train_data/train.txt')
#使用的是txt格式img_path,xmin,ymin,xmax,ymax,class_name训练还是voc的xml格式训练，simple表示txt，pascal_voc表示是xml
...
if 'bg' not in classes_count: #添加一个背景类别
    classes_count['bg'] = 0
...
random.shuffle(all_imgs) #打乱所有的训练样本
...
data_gen_val = data_generators.get_anchor_gt(test_imgs, classes_count, C, nn.get_img_output_length, K.image_data_format(), mode='val')
...
shared_layers = nn.nn_base(img_input, trainable=True)#共享特征层
...
for epoch_num in range(0,num_epochs):
    ...
    progbar = Progbar(epoch_length)   # keras progress bar
