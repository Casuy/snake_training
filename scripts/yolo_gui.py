...
class goods_counting_gui:
    ...
    def run_classification(self):
        ...
        label_dict = {0: "BandyBandy", 1: "CarpetPython", 2: "CoastalTaipan",
                      3: "CommonDeathAdder", 4: "EasternBrownSnake", 5: "LowlandCopperhead",
                      6: "MulgaSnake", 7: "RedBelliedBlackSnake", 8: "SpottedPyhton",
                      9: "TigerSnake", 10: "WesternBrownSnake"}
        ...
        box_coords = (
        (text_offset_x - 5, text_offset_y + 5), (text_offset_x + text_width + 5, text_offset_y - text_height - 5))
        cv2.rectangle(plot, box_coords[0], box_coords[1], rectangle_bgr, cv2.FILLED)
        cv2.putText(plot, text, (text_offset_x, text_offset_y), font, fontScale=font_scale, color=(0, 0, 0),
                    thickness=1)
    ...
    def run_detection(self):
        weightsPath = "yolo/yolov3_custom_train_final.weights"
        configPath = "yolo/yolov3_custom_train.cfg"
        labelsPath = "yolo/yolo.names"
        LABELS = open(labelsPath).read().strip().split("\n")  # Category
        COLORS = np.random.randint(0, 255, size=(len(LABELS), 3), dtype="uint8")  # Color
        COLORS = [(0,255,0), (255,0,0), (0,0,255), (0,255,0), (255,0,0), (0,0,255),(0,255,0), (255,0,0), (0,0,255),(0,255,0), (255,0,0)]
        boxes = []
        confidences = []
        classIDs = []
        ...
        if len(idxs) > 0:
            for i in idxs.flatten():
                ...
                text = "{}: {:d}".format(LABELS[classIDs[i]], int(confidences[i]*100))
                (retval, baseLine) = cv2.getTextSize(text, cv2.FONT_HERSHEY_COMPLEX, 1, 1)
                cv2.rectangle(image, (x + 5, y + baseLine + 35), (x + retval[0], y - retval[1] + 35), (0, 0, 0), 2)
                cv2.rectangle(image, (x + 5, y + baseLine + 35), (x + retval[0], y - retval[1] + 35), (255, 255, 255), -1)
                cv2.putText(image, text, (x + 10, y + 40), cv2.FONT_HERSHEY_DUPLEX, 1, (0,0,0), 1)
