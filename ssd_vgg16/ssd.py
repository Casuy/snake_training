...
from tensorflow.python.keras.backend import set_session,get_session,get_graph

class SSD(object):
    _defaults = {
        "model_path": 'logs/ep106-loss0.207-val_loss0.855.h5',  # 此路径可以修改为自己的模型目录
    ...
    def generate(self):
        ...
        # 否则先构建模型再载入
        try:
            self.ssd_model = tf.keras.models.load_model(model_path, compile=False)
        except:
    ...
    def detect_image(self, image):
        ...
        self.predict_all = []
        # 图片预处理，归一化
        photo = tf.keras.applications.imagenet_utils.preprocess_input(np.reshape(photo,[1,300,300,3]))
        #self.ssd_model.summary()
        ...
        for i, c in enumerate(top_label_indices):
            ...
            self.result_ = '{} {} {} {} {} {}'.format("".join(predicted_class.split(" ")), score, left, top, right, bottom)
